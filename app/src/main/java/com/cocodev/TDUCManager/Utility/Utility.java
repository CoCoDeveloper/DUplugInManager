...
/**
 * Created by Sudarshan on 07-07-2017.
 */
 ...
 */
    ...
    public static String getTimeAgo(Context context,long time) {
        ...
        int julianDay = Time.getJulianDay(time, t.gmtoff);
        int currentJulianDay = Time.getJulianDay(now, t.gmtoff);

        if(julianDay<currentJulianDay-15){
            return formatDate(time);
        }

        if( time > now ){
            return getFriendlyDayString(context,time);
        }
